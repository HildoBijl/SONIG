...
for i = 1:sonig.dy
	q(:,i) = sonig.hyp.ly(i)^2/sqrt(det(xDist.cov)*det(inv(xDist.cov) + diag(1./sonig.hyp.lx(:,i).^2)))*exp(-1/2*sum((diff'/(diag(sonig.hyp.lx(:,i).^2) + xDist.cov)).*diff', 2));
	for j = 1:sonig.dy
		xn = repmat(permute(diffNormalized(:,:,i),[1,3,2,4]),[1,1,1,sonig.nu]) + repmat(permute(diffNormalized(:,:,j),[1,3,4,2]),[1,1,sonig.nu,1]);
		Q(:,:,i,j) = sonig.hyp.ly(i)^2*sonig.hyp.ly(j)^2/sqrt(det(xDist.cov)*det(inv(xDist.cov) + diag(1./sonig.hyp.lx(:,i).^2) + diag(1./sonig.hyp.lx(:,j).^2)))*(exp(-1/2*sum(diffNormalized(:,:,i).*diff,1))'*exp(-1/2*sum(diffNormalized(:,:,j).*diff,1))).*permute(exp(1/2*mmat(mmat(permute(xn,[2,1,3,4]), repmat(inv(inv(xDist.cov) + diag(1./sonig.hyp.lx(:,i).^2) + diag(1./sonig.hyp.lx(:,j).^2)), [1,1,sonig.nu,sonig.nu])), xn)),[3,4,1,2]);
	end
end
