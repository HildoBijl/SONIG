...
counter = 0;
while counter < 30 && (exist('prevxl','var') == 0 || sum(((xl-prevxl)./smallestLengthScales).^2) > 0.001) % We continue iterating until the change between xl and the previous xl (when normalized) is below a certain threshold. We also have a security check on the maximum number of iterations.
...
% 		dKux = permute(dKxu,[2,1,3]); % This line has been deemed unnecessary.
	...
	inputCov = inv(dmux'/(Sxx + realOutputCov)*dmux + inv(input.cov));
...
adjustedOutput = createDistribution(output.mean + covYX/input.cov*(newInput.mean - input.mean), realOutputCov);
...
% Next, we start calculating the derivatives of all important parameters. For this, we use the definition of the squared exponential covariance function.
...
% The next step is to calculate the mean and covariance, and their derivatives, of the posterior output.
